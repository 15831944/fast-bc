...
TEST_CASE("SubGraph contructor and getters", "[fastbc]")
{
	...
	REQUIRE_THROWS(subGraph = std::make_shared<SubGraph<int, double>>(std::vector<int>({ 0, 1, 2, 3, 6 }), graph));
#endif

	REQUIRE_NOTHROW(subGraph = std::make_shared<SubGraph<int, double>>(std::vector<int>({ 0, 1, 2, 3, 4 }), graph));
